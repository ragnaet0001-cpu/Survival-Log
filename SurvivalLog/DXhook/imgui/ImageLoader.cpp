...
std::optional<ImageLoader::ImageData> ImageLoader::GetImage(const std::string& imageName, const char* type)
{
	...
	if (!loadResult)
	{
		LOG_WARNING("Failed to load image: %s from resources", imageName.c_str());
		return {};
	}
	...
	bool textureResult = D3D11::GetInstance().LoadTextureFromMemory(image_data, image_width, image_height,
...
std::optional<ImageLoader::GIFData*> ImageLoader::GetGIF(const std::string& imageName)
{
	...
	int frame_count = 0;
	int* delays = nullptr;
	unsigned char* image_data = stbi_load_gif_from_memory(pDestination, size, &delays, &image_width, &image_height, &frame_count, nullptr, 4);
	if (image_data == NULL || !frame_count)
	{
		LOG_WARNING("Failed to convert GIF:'%s' frame_count:'%d'to RGBA buffer by 'stb_image.h' Error:'%s'", imageName.c_str(), frame_count , stbi_failure_reason());
		return {};
	}
	...
	for (int i = 0; i < frame_count; ++i)
	{
		...
		int width, height;
		bool textureResult = D3D11::GetInstance().LoadTextureFromMemory(((unsigned char*)image_data) + nextFrameDataOffset, image_width, image_height,
